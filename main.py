from opsmate.knowledgestore.models import init_table, init_db, KnowledgeStore
...
async def categorize(text: str) -> str:
    ...
    Please categorise the text into one or more unique categories using the following list:
    {", ".join([cat.value for cat in list(Category)])}
    """
    return text
...
async def main():
    ...
    ingestor = FsIngestion(
        ...
        local_path=".",
        glob_pattern="*.md",
        post_chunk_hooks=[summarize_chunk, categorize_chunk],
    )
    ...
    async for chunk in ingestor.ingest():
        ...
        # print(f"Categories: {chunk.metadata['categories']}")
